...
from apps.story.models import DayStory, MonthStory, Video, YearStory
...
class RequestPostYearStorySerializer(serializers.Serializer):
    year = serializers.IntegerField(
        required=True,
        allow_null=False,
    )
    ...
    def create(self) -> DayStory:
        ...
        if not videos_paths:
            raise serializers.ValidationError("Отсутствуют видео за этот год")
        ...
        # Удалить предыдущую историю за этот месяц
        YearStory.objects.filter(
            year=self.validated_data.get("year"),
            user=user,
        ).delete()

        return YearStory.objects.create(
            year=self.validated_data.get("year"),
            video=video,
            user=user,
        )
...
class MonthStorySerializer(serializers.ModelSerializer):
    video_id = serializers.SerializerMethodField(
        source="get_video_id",
    )
...
class YearStorySerializer(serializers.ModelSerializer):
    video_id = serializers.SerializerMethodField(
        source="get_video_id",
    )
